...
| `user_turn_not_ready` | 空の入力欄を証明できない (モーダル・下書き・読めない画面)。 | なし — 阻害要因を解消して再試行 |
| `user_turn_unsupported_target` | そもそも Claude / Codex が動いていないペイン。 | なし |
| `user_turn_invalid_body` | 本文が空 / 制御文字入り / 複数行なのに相手が bracketed paste 未対応。 | なし |
