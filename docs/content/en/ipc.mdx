...
`tab` (0-based index of the tab the pane lives in) and `tab_name` (that
tab's display label) are **display metadata only** — indexes shift when
tabs close and labels are not unique, so a pane's `id` stays the only
tab-stable address. `cwd`, not `name`, is what tells apart panes
belonging to two independent orchestrations running in different tabs:
each of them has a `dispatcher` and a `worker-<task_id>`.

The IPC request behind this command, `list`, takes an optional `tab`
selector choosing which tab(s) to enumerate (Issue #329):
`{"name": "workers"}` / `{"index": 2}` / `{"pane_id": 17}` — the same
three shapes the spawn-side tab selector uses, so the codes for an
unresolvable one are the same too (`tab_not_found` / `tab_ambiguous` /
`pane_not_found`) — or the bare string `"all"` for every tab, the
caller's own first and the rest in index order. Omitting it preserves
the prior behavior exactly, and `{"cmd":"list"}` is byte-identical on
the wire to what pre-#329 clients sent. `renga-cp list` never sends it —
the CLI has no tab flag and still lists the active tab — but the MCP
`list_panes` tool exposes it (spelled `{"all": true}` there); see
[Peer messaging](/peer-messaging). When a request carries both `tab`
and a `from_pane`, each record also carries `same_tab`.

A `list` that carries `tab` requires a server advertising the
`cross_tab_list` capability and is refused with `[server_too_old]`
otherwise. Unlike `subscribe_pane_scope` below, this one has to be
gated: a server predating #329 drops the unknown field and answers with
the caller's tab alone — a well-formed reply a client cannot tell apart
from a correct one, which would make an orchestrator retire live panes
and under-count its capacity. A `list` without `tab` is unchanged on
every server and gates on `caller_scope` as before.
...
```bash
renga-cp send --name worker-foo --enter "git status"
renga-cp send --id 2 "Hello"
renga-cp send --focused $'\x1b'  # send Esc
```
...
```bash
renga-cp split --direction vertical --command "claude" --id worker-a --role worker
```
...
```bash
renga-cp new-tab --command "claude --permission-mode plan" --id worker-plan --role worker --label "plan-session"
```
...
```bash
renga-cp events --timeout 5s --count 10
```
...
- `events_dropped`: number of events the server had to drop for this subscriber (slow consumer signal; reconcile with `renga-cp list` if you see these).
...
`renga-cp events` sends no `from_pane`, so the CLI stream is unchanged — it still carries `peer_inbox` lines for every pane, as it always has. `from_pane` is an opt-in for the other kind of consumer: an integration that speaks IPC on behalf of a single pane, like the bundled `renga-cp mcp-peer`, which only ever acts on the messages addressed to the pane it serves. Declining costs nothing, and the server never guesses a pane for a subscription that named none — neither the `hello` pid nor an earlier peer registration belongs to the connection doing the subscribing, so a guess would eventually bind the wrong pane. Peer messaging itself is untouched either way: messages reach the recipient pane the usual way (a `<channel>` tag for Claude, a `check_messages` nudge for Codex and Copilot).
...
```bash
renga-cp inspect --name worker-foo --lines 10 --cursor
```
...
| `target_too_small` | Split refused because the **target pane** is too small: halving it would fall below `min_pane_width` / `min_pane_height`. Target-local — the message reports the target's geometry, the minimum required and the tab's pane count, so retrying against a bigger target (or the other direction) can succeed. |
| `pane_limit_reached` | Split refused because the **tab** already holds MAX_PANES (16) panes. Tab-global — no target will help; close a pane or spawn into another tab. |
| `split_refused` | Split refused for neither of the two causes above — from 3.0 only a terminal below the layout threshold. A 2.x server answers *both* causes above with this code, so unless you have confirmed the server advertises `split_refusal_causes`, read it the old way: cause unknown (#335). |
| `io_error` | PTY write / spawn / OS-level failure. |

Shell-side dispatch example:

```bash
out=$(renga-cp send --name worker-foo --enter "ping" 2>&1)
case "$out" in
...
`renga-cp events` subscribers are released promptly when the client half-closes the connection. The server emits a heartbeat every 30 seconds, so the next write into a dead socket fails and unsubscribes the slot — even on quiet workspaces that otherwise go minutes without an event.
