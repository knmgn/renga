...
If you want a Codex worker, make sure `renga mcp install --client codex` has been run once, then launch plain `codex` in a pane or let an orchestrator call `spawn_codex_pane`. A GitHub Copilot CLI worker is the same shape one client over: `renga mcp install --client copilot`, then plain `copilot` or `spawn_copilot_pane`. Copilot asks for folder trust — and, in a directory it has seen before, which session to restore — before its composer appears; renga will not answer those dialogs for you, so clear them once with `send_keys` or by hand.
