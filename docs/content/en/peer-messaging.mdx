...
| `user_turn_not_ready` | No empty composer could be proven: a modal, a draft, or a screen renga cannot read. | No — clear the blocker and retry |
| `user_turn_unsupported_target` | The pane is not running Claude or Codex at all. | No |
| `user_turn_invalid_body` | Empty body, control characters, or multi-line without bracketed paste on the target. | No |
