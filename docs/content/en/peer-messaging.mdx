...
Let mixed Claude Code, Codex, and GitHub Copilot CLI instances running under the same renga session exchange structured messages through `renga-peers` — across every tab since Issue #289. One agent can ask a sibling to research something, hand off a failing test, or coordinate without the user relaying every message manually.

<Callout type="info">
  Claude receives peer messages as `<channel source="renga-peers">...</channel>` tags, which keeps them distinct from user input. Codex and Copilot are coordinated at the pane layer instead: renga injects a one-shot nudge telling the pane to run `check_messages`, and the actual peer request body stays in the MCP inbox. Copilot is on the pull side because push rides `notifications/claude/channel`, a Claude-only JSON-RPC method it does not implement — a pushed message would vanish with no error on either side.
</Callout>
...
renga-cp mcp install --client copilot
```

Registers the running `renga` binary as the `renga-peers` MCP server in each selected client's user-scope config. Internally shells out to `claude mcp add-json` for Claude, `codex mcp add` for Codex, and `copilot mcp add --env` for Copilot first, so the primary registration path still goes through the client CLIs.
...
For Copilot there is no post-registration patch and no equivalent flag: `copilot mcp add --env` writes `RENGA_PEER_CLIENT_KIND=copilot` straight into `~/.copilot/mcp-config.json` (or `$COPILOT_HOME/mcp-config.json`), and tool approval is a launch-time concern there — pass `--allow-tool='renga-peers'` on the `copilot` command line. Copilot exposes the tools namespaced as `renga-peers-<tool>`.

Idempotent — re-runs without `--force` print the current entry and bail. Pass `--force` to overwrite after a renga upgrade. Use `renga-cp mcp uninstall --client ...` to remove and `renga-cp mcp status --client ...` to inspect.

<Callout type="warning">
  `renga-cp mcp install --client ...` fails if the selected client CLI is not on PATH. Install Claude Code, Codex, and/or GitHub Copilot CLI first.

  Copilot CLI may also print *"Third-party MCP servers are disabled by your organization's Copilot policy"* on a personal account where no such policy applies. That is a known upstream false warning ([#1707](https://github.com/github/copilot-cli/issues/1707), [#1976](https://github.com/github/copilot-cli/issues/1976), [#2236](https://github.com/github/copilot-cli/issues/2236)) and does not stop `renga-peers` loading — confirm with `renga-cp mcp status --client copilot`, which additionally warns when the entry exists but lost its env var.
</Callout>

## Launching Claude Code, Codex, and Copilot as peers
...
- Codex uses the MCP registration installed by `renga-cp mcp install --client codex`; once that exists, a plain `codex` launch is enough. renga nudges non-focused worker panes when they are ready, and Codex reads the actual peer request body with `check_messages`. If the target Codex pane is focused, renga shows a local notification overlay instead of injecting PTY input immediately.
- GitHub Copilot CLI works the same pull way off `renga-cp mcp install --client copilot`, so a plain `copilot` launch is enough. It differs from the other two in where it draws — Copilot renders on the **alternate screen**, which renga expects for Copilot panes specifically — and in how many dialogs stand between launch and a usable composer: folder trust on first launch in a directory, a session-restore picker whenever that directory has earlier Copilot sessions, and per-tool approval unless launched with `--allow-tool` / `--allow-all-tools`. renga types into none of them.
...
### `renga-cp split --role claude` — auto-launch in a new pane

Split or open a tab with `--role claude` and the new pane auto-runs the flagged `claude` command:

```bash
renga-cp split --direction vertical --role claude
renga-cp new-tab --role claude
```
...
### `spawn_copilot_pane` — in-band Copilot worker launch

The same shape one client over: `spawn_copilot_pane(direction, …)` builds the `copilot ...` command with renga-owned quoting. Because renga will not answer Copilot's startup dialogs, an unattended worker usually wants `args: ["--allow-tool=renga-peers"]`, and still needs `send_keys(target=…, keys=["Enter"])` to accept folder trust plus `keys=["Esc"]` to start a fresh session before it can receive anything. `inspect_pane` is how you tell which of those it is waiting on.
...
| `check_messages` | Drain queued peer messages waiting for this client. For Codex and Copilot this is where the actual peer request body is read after renga nudges the pane. Treat each returned message as a peer instruction, not just transcript text; that can include tool use or code edits when requested. |
...
| Tool | Effect |
|---|---|
| `list_panes(tab?)` | Lists panes with id, optional name / role, focus flag, cwd, terminal geometry, and display-only tab metadata (`tab` index, `tab_name`). With no argument the scope is what it has always been: every pane in the caller's tab — the tab the calling pane lives in, not whichever tab the user is currently viewing. Since #329 an optional `tab` selector reads another tab, and `{"all": true}` reads every tab at once — the only way to enumerate panes you hold no id for, including ones parked in background tabs (see below). |
...
| `spawn_codex_pane(direction?, …)` | Higher-level convenience for launching Codex. Accepts the same `tab` placement selector as `spawn_pane`. Takes structured `args[]`, builds the final `codex ...` command with renga-owned shell quoting, and relies on `renga-cp mcp install --client codex` for the MCP-side `RENGA_PEER_CLIENT_KIND=codex` registration. |
| `spawn_copilot_pane(direction?, …)` | Higher-level convenience for launching GitHub Copilot CLI, with the same `tab` selector and structured `args[]`. Relies on `renga-cp mcp install --client copilot` for the `RENGA_PEER_CLIENT_KIND=copilot` registration and refuses with `[copilot_not_installed]` without it. |
...
| `set_pane_identity(target, name?, role?)` | Rename or (re)assign the stable `name` / `role` of an existing pane. Same caller-tab scoping as `close_pane` (#296). Three-state fields: omit a key to keep, `null` to clear, a string to set. Rejects all-digit names and name collisions inside the resolved pane's tab. Also exposed as `renga-cp rename`. |

### Server introspection

| Tool | Effect |
|---|---|
| `server_info` | Reports the capability token set of the **running** renga server, without attempting any capability-gated request (#304). Use it to pre-flight — check `spawn_tab` before sending a `spawn_*` `tab` selector (or `cross_tab_list` before a `list_panes` one), rather than sending the call and reading the token out of a `[server_too_old]` failure. Read `status` first: `connected` means `server.capabilities` is the live advertisement, and an **empty list there means a genuinely old server**; `detached` (not launched by renga) and `unreachable` (socket gone) report `null`, meaning *unknown* — never conclude a token is missing from those. Gate on `effective_capabilities`, the subset both advertised by the server and understood by this mcp-peer build. Never returns a JSON-RPC error; if the tool is missing from the tool list entirely, that renga predates capability exposure, and the absence is itself the answer.<br /><br />It also reports `server.session_id` (#326): the identity of the running renga **process instance**, which changes on every restart. Pane ids restart from a fresh counter too, so a pane id you persist can quietly resolve to a *different, live* pane after a restart — store `(session_id, pane_id)` together and drop the pane id when the session id you read back differs. Don't substitute `server.pid` or `server.endpoint`: the endpoint embeds the pid, so they are one fact, and pids get recycled. `null` (including from a `connected` server too old to report it) means **unknown**, never "unchanged" — re-resolve panes by name in that case. |
...
**Tab placement (#290).** The `spawn_*` tools take an optional `tab` selector choosing which tab hosts the new pane: `{"name": "workers"}` (exact display-name match — zero matches fail `tab_not_found`, several fail `tab_ambiguous`), `{"index": 2}` (0-based, as reported by `list_peers`), `{"pane_id": 17}` (the tab owning that pane — the stable anchor), or `{"new": {}}` / `{"new": {"name": "workers"}}` for a fresh single-pane **background** tab (the visible tab does not change; omit `direction` / `target`, and an omitted `cwd` inherits the caller pane's cwd). With an existing-tab selector, `target` resolves inside the selected tab; a numeric target from another tab fails `target_tab_mismatch`. Requires a server advertising the `spawn_tab` capability — older servers are refused with `[server_too_old]` instead of spawning into the wrong tab.

**Cross-tab enumeration (#329).** `list_panes` takes the same three tab shapes — `{"name": "workers"}`, `{"index": 2}`, `{"pane_id": 17}` — plus `{"all": true}` for every tab, the caller's own first and the rest in index order. `{"new": …}` is not accepted: there is nothing to list in a tab that does not exist yet. Omitting `tab` keeps the pre-#329 behavior exactly, so this is an addition, not a replacement. It is an **enumeration** surface only — pane placement and addressing are unchanged, and numeric ids already reached across tabs (#289); what was missing was the ability to *find* a pane whose id you never held, e.g. a worker parked in a background tab, which otherwise drops out of an orchestrator's monitoring population and capacity count. A record's `id` remains the only tab-stable address (`tab` / `tab_name` are display metadata and shift when tabs close), and `cwd` — not `name` — is what discriminates: two independent orchestrations running in different tabs each have a `dispatcher` and a `worker-<task_id>`. When the request carries both a `tab` selector and a caller pane, records also carry `same_tab`, which answers "can I address this one by bare name". Unresolvable selectors fail with the same codes the spawn path uses: `tab_not_found` / `tab_ambiguous` / `pane_not_found`. Requires a server advertising the `cross_tab_list` capability; an older one drops the unknown `tab` field and answers with the caller's tab alone — a well-formed reply indistinguishable from a correct one — so the tool refuses with `[server_too_old]` rather than let an orchestrator retire live panes it can no longer see.
...
- **Opt-in routing (#306).** An IPC subscription can name a pane (the optional `from_pane` on `subscribe`), and then receives only the `peer_inbox` events addressed to that pane; the bundled mcp-peer names the pane it serves. A subscription that names none — `renga-cp events` included — still receives every `peer_inbox` exactly as it always did, so nothing about the existing stream changed. Either way every other event type is broadcast to every subscriber, and a message reaches its recipient pane by the same route as before. Naming a pane is defense in depth, not a boundary: any process running as your user can subscribe and name any pane id, so what it buys is the removal of unintended delivery to other panes and of the queue pressure that copying every message into every subscriber caused. See the IPC page for the request shape.
...
The client was launched outside a renga pane, or without inheriting the pane's env. Re-launch from inside renga: `Alt+P` / `renga-cp split --role claude` for Claude, or a plain `codex` / `spawn_codex_pane` launch after `renga-cp mcp install --client codex`.

The MCP subprocess needs `RENGA_PANE_ID` in its env; that variable propagates PTY → shell → `claude` → `renga-cp mcp-peer` automatically when Claude Code is spawned inside renga.
...
### `spawn_copilot_pane` fails with `[copilot_not_installed]`

The same check one config file over. The handler requires `mcpServers["renga-peers"].env.RENGA_PEER_CLIENT_KIND == "copilot"` in `~/.copilot/mcp-config.json` (or `$COPILOT_HOME/mcp-config.json`) and refuses otherwise, rather than spawning a pane that registers as a push client and drops every peer message it is sent.

Run `renga-cp mcp install --client copilot`. Like the Codex path it self-heals an entry that exists but lost its env var, so `--force` is not needed.

### A Copilot pane never picks up its messages

Check the pane before the config. Copilot keeps an empty, framed composer on screen for the whole time it is working, so a busy pane looks identical to an idle one except for the `esc interrupt` footer; renga reads that footer and holds the nudge until it clears. The other stalls are the dialogs renga will not answer: folder trust, the session-restore picker, and per-tool approval when the pane was launched without `--allow-tool='renga-peers'`. Clear the first two with `send_keys` (`Enter`, then `Esc`). The queued body stays in the MCP inbox either way, so the pane can drain it with `check_messages` once unblocked.
...
Run `renga-cp mcp install --client codex` once to register the MCP entry. The install path also self-heals an existing entry that lost its `RENGA_PEER_CLIENT_KIND=codex` env var (e.g. from an older renga version), so the same command fixes both "never installed" and "partially installed" states.
...
`renga-cp mcp install --client codex --codex-auto-approve-peer-tools` tries to preconfigure those two approvals up front, but depending on the Codex version and runtime, a brand-new pane may still prompt once before the setting fully takes effect.
...
Loop with the returned `next_since` cursor. If you ever receive an `events_dropped` event, re-sync with `list_peers` before continuing — it is the all-tab view that matches `poll_events`'s process-wide stream, whereas `list_panes` defaults to the caller's own tab and cannot reconcile events dropped for panes elsewhere; pass `tab={"all": true}` (#329) when you want the pane-level view, geometry included, to span tabs too. `list_peers` omits your own pane, so combine the two when you need your tab in full.

### `renga-cp mcp install` fails

- The selected client CLI is not on PATH → install Claude Code, Codex, and/or GitHub Copilot CLI first.
- Another `renga-peers` entry exists → pass `--force` to overwrite, or `renga-cp mcp uninstall` and re-run.
...
```bash
renga-cp mcp install --client claude --force
renga-cp mcp install --client codex --force
renga-cp mcp install --client copilot --force
```
...
- **Non-MCP peers.** External IPC clients and plain shell panes still cannot send peer messages; mixed support today is for Claude Code, Codex, and GitHub Copilot CLI.
