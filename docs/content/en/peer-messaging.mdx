...
| `user_turn_invalid_body` | Empty body, control characters, a tab in a single-line body (the recipient reads it as a Tab keypress, not text), multi-line without bracketed paste on the target, a body over 4 KiB (a user turn is a prompt, not a file — `deliver="channel"` has no such limit), or — for **Codex** targets only — a body too long to fit on one composer row. | No |
