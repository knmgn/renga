...
renga mcp install --client copilot
```
...
<Callout type="warning">
  `renga mcp install --client ...` fails if the selected client CLI is not on PATH. Install Claude Code, Codex, and/or GitHub Copilot CLI first.

  Copilot CLI may also print *"Third-party MCP servers are disabled by your organization's Copilot policy"* on a personal account where no such policy applies. That is a known upstream false warning ([#1707](https://github.com/github/copilot-cli/issues/1707), [#1976](https://github.com/github/copilot-cli/issues/1976), [#2236](https://github.com/github/copilot-cli/issues/2236)) and does not stop `renga-peers` loading — confirm with `renga mcp status --client copilot`, which additionally warns when the entry exists but lost its env var.
</Callout>
...
- GitHub Copilot CLI works the same pull way off `renga mcp install --client copilot`, so a plain `copilot` launch is enough. It differs from the other two in where it draws — Copilot renders on the **alternate screen**, which renga expects for Copilot panes specifically — and in how many dialogs stand between launch and a usable composer: folder trust on first launch in a directory, a session-restore picker whenever that directory has earlier Copilot sessions, and per-tool approval unless launched with `--allow-tool` / `--allow-all-tools`. renga types into none of them.
...
| `spawn_copilot_pane(direction?, …)` | Higher-level convenience for launching GitHub Copilot CLI, with the same `tab` selector and structured `args[]`. Relies on `renga mcp install --client copilot` for the `RENGA_PEER_CLIENT_KIND=copilot` registration and refuses with `[copilot_not_installed]` without it. |
...
Run `renga mcp install --client copilot`. Like the Codex path it self-heals an entry that exists but lost its env var, so `--force` is not needed.
...
renga mcp install --client copilot --force
```
