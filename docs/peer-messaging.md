...
> **Tab scope.** For `list_panes`, `spawn_pane`, `spawn_claude_pane`, `spawn_codex_pane`, `focus_pane`, `inspect_pane` and `send_keys`, "the current tab" means **the tab your own pane lives in**, not whichever tab the user happens to be looking at. Relative selectors (`target="focused"`, a stable pane name) never leave your tab; an explicit numeric pane id may reach a pane in another tab. `focus_pane` additionally switches the tab the user is *viewing* whenever the resolved pane is not already in it, because focus the keyboard cannot reach is not focus.
>
> **`close_pane` and `set_pane_identity` are the exceptions** — they were already cross-tab before Issue #288 and still resolve `focused` and names against the tab the **user is viewing**. `close_pane(target="focused")` from a background tab therefore terminates a pane in the user's visible tab. Pass an explicit numeric id to those two.
>
> The seven were fixed in Issue #288; before it they resolved against the visible tab, so an orchestrator running in a background tab would quietly `send_keys` into whatever the user had switched to. If a tool now answers `[server_too_old] ... restart renga`, the renga *process* predates the fix even though the binary on disk does not — restart renga.
