...
> **Tab scope.** "The current tab" for these tools means **the tab your own pane lives in**, not whichever tab the user happens to be looking at. Relative selectors (`target="focused"`, a stable pane name) never leave your tab. An explicit numeric pane id may reach a pane in another tab — the same cross-tab escape hatch `close_pane` / `set_pane_identity` already had. One exception is worth calling out: `focus_pane` switches the tab the user is *viewing* whenever the resolved pane is not already in it, because focus the keyboard cannot reach is not focus.
>
> This was fixed in Issue #288; before it, every one of these tools resolved against the visible tab, so an orchestrator running in a background tab would quietly `send_keys` into whatever the user had switched to. If a tool now answers `[server_too_old] ... restart renga`, the renga *process* predates the fix even though the binary on disk does not — restart renga.
